...
sf::Vector2f saturateVector(sf::Vector2f vector, float threshold)
{
    ...
    result.x = std::min (abs(vector.x), threshold);
    result.y = std::min (abs(vector.y), threshold);
...
{
    sf::Vector2f result;
    double phi = static_cast<double>(toRadian(theta));

    result.x = base.x*static_cast<float>(cos(phi)) - base.y*static_cast<float>(sin(phi));
    result.y = base.x*static_cast<float>(sin(phi)) + base.y*static_cast<float>(cos(phi));
...
{
    float angle = static_cast<float>(atan2(static_cast<double>(vector.x), static_cast<double>(-vector.y)));
...
sf::Vector2f fromAngle(float theta)
{
    ...
    result.x = static_cast<float>(cos(phi));
    result.y = static_cast<float>(sin(phi));
